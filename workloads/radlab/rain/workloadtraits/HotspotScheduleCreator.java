...
/* Simple diurnal signal - 24 data points - with a data hotspot. 
 For the data hotspot we keep the number of hot objects fixed, but vary the hot traffic fraction
 directed at them. By not specifying the hot set explicitly the membership of the hot set
 can change from interval to interval but the total number of hot objects won't.
 */
public class HotspotScheduleCreator extends LoadScheduleCreator {
	...
	private int[] _numHotObjects = { 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100,
			100, 100 };

	private double[] _hotTrafficFraction = { 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64, 0.64,
	...
	public LinkedList<LoadDefinition> createSchedule(String target, JSONObject config) throws JSONException {
		...
		if (this._relativeLoads.length != this._numHotObjects.length)
			throw new JSONException("Length of Number of hot objects array: " + this._numHotObjects.length
					+ " is not the same as relative load array: " + this._relativeLoads.length);

		if (this._relativeLoads.length != this._hotTrafficFraction.length)
			throw new JSONException("Length of hot traffic fraction array: " + this._hotTrafficFraction.length
					+ " is not the same as relative load array: " + this._relativeLoads.length);
		...
		for (int i = 0; i < this._relativeLoads.length; i++) {
			...
			if (i == 0) {
				...
				profileConfig.put(StorageLoadProfile.CFG_NUM_HOT_OBJECTS_KEY, this._numHotObjects[i]);
